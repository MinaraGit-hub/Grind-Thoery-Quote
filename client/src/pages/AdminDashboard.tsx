...
export default function AdminDashboard() {
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      toast({
        ...
        description: "Hourly rates and event surplus have been updated successfully.",
      });
    } catch (error) {
      toast({
        title: "Error",
  ...
  const handleSurplusChange = (type: string, value: string) => {
    const numValue = parseInt(value) || 0;
    setSurplus(prev => ({
      ...prev,
      [type]: numValue
              ...
              <div className="space-y-8">
                ...
                <div className="border-t border-gray-100 pt-6">
                  <h3 className="font-semibold text-gray-900 mb-4 flex items-center gap-2">
                    Event Type Surplus (%)
                    <span className="text-xs font-normal text-gray-500 bg-gray-100 px-2 py-1 rounded-full">Adjusts total cost</span>
                  </h3>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Object.keys(surplus).map((type) => (
                      <div key={type} className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                        ...
                        <div className="relative">
                          ...
                          <input
                            type="number"
                            value={surplus[type]}
                            onChange={(e) => handleSurplusChange(type, e.target.value)}
